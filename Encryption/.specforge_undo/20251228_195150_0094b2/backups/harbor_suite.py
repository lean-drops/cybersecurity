...
class ConfigPanel:
    def __init__(self, grid_layout, label_factory: Callable[[str], Any], make_input_str: Callable[[str], Any], make_input_int: Callable[[], Any]):
...
def main() -> None:
    ...
    class SendTab(QWidget):
        def __init__(self) -> None:
            ...
            self.method.setReadOnly(True)
            self.method.setObjectName("Input")
            ...
            compose.body.addWidget(QLabel("Encryption plugins (from encryption_methods)"))
            ...
            self.cfg_panel = ConfigPanel(
                self.cfg_grid,
                label_factory=lambda s: _mk_label(s),
                make_input_str=lambda t: _mk_lineedit(t),
                make_input_int=lambda: _mk_spinbox(),
            )
            ...
            self.plain.setPlaceholderText("Type plaintext...")
        ...
        def _on_plugin_selected(self) -> None:
            ...
            self.method.setText(f"{pl.method_name} ({pl.method_id})")
    ...
    class FrequencyTab(QWidget):
        def __init__(self) -> None:
            ...
            attacks.body.addWidget(self.use_record_cfg)
            attacks.body.addWidget(self.freq_sel)
            attacks.body.addWidget(self.freq_all)
            attacks.body.addWidget(self.crack_caesar)
            attacks.body.addWidget(self.crack_subst)

            left.addWidget(attacks, 1)
            root.addLayout(left, 2)
        ...
        def _on_record_selected(self) -> None:
            ...
            # When record changes, try to auto-pick matching plugin.
            self._auto_pick()
        ...
        def _decrypt(self) -> None:
            ...
            if self.use_record_cfg.isChecked() and isinstance(r.get("config", {}), dict):
                # record config wins unless user edited fields (we still merge: panel overrides record)
    ...
    class Suite(QWidget):
        def __init__(self) -> None:
            ...
            subtitle = QLabel("Send, analyze, and tamper with messages (plugins everywhere).")
            ...
            self.tabs = QTabWidget()
            self.tabs.setObjectName("Tabs")
            self.tabs.addTab(SendTab(), "Send")
            self.tabs.addTab(FrequencyTab(), "Frequency attack")
            self.tabs.addTab(ModificationTab(), "Modification")
